...
**Cut order if time runs out**, carried over from the original proposal: tier badges first, then extra neighborhoods (keep campus + one Oakland chip), then — everything else before touching confirm. Confirm is P0 per §4 and is never cut, full stop; the map is never cut. (The swipe interaction *was* on that never-cut list — see §4C, it's being replaced.)

---

## 4C. Reciprocity chain + carousel (approved, user-directed)

1. **Reciprocity chain visualization.** The pitch's actual differentiator (QUEST-CONTEXT-DUMP.md §9) is real reciprocity, not a karma number — show the literal chain: A helped B, B helped C, C helped A, three strangers, closed loop, tonight. Every `CONFIRMED` quest already has `helperId`/`requesterId`/`helperName`/`requesterName`/`updatedAt` in Mongo — this is a read-only aggregation over existing data, no new mutation logic, no new sponsor.
   - Bundle: B (new read query + pure chain-finding function) + D (new display component, wired into the existing Wallet/"You" screen — **not a fifth screen**, §1's four-screen limit holds). Time-box: 60 min. Fallback: if fewer than 2 confirmed quests exist yet, or no closed loop has formed, show the longest open chain found so far instead of insisting on a full loop — never show a broken/empty panel. Visible in 3 seconds: yes, this is the whole point.
2. **Swipe → carousel.** `SwipeDeck`'s drag-to-accept/-skip gesture is redundant — "I'll take it" / "Not now" buttons already exist inside every card. Replace the pointer-drag physics with a simple scroll-snap carousel; the buttons stay as the only accept/skip actions.
   - Bundle: D only. Time-box: 30 min. Fallback: none needed, this is a pure simplification with no new failure mode. Supersedes the old "swipe is never cut" line above.
...
| 2026-09-12 | Approved §4C: a reciprocity-chain visualization (read-only, off existing `CONFIRMED` quest data) and replacing `SwipeDeck`'s drag gesture with a scroll-snap carousel | User call: the karma number alone doesn't show the actual differentiator (real closed-loop reciprocity between strangers); swipe was redundant with the in-card CTA buttons | user |
| | | | |
